...
layout = [  
    ...
    [sg.Text('Выбор протокола', size=(15, 1)), sg.InputCombo(("Привзяка к биту на основе симметричной криптографии","Протокол подбрасывания монеты на основе однонаправленной функции","Экспоненциальный протокол подбрасывнания монеты","ЭЦП"), size=(70, 3))],
    [sg.Submit(), sg.Cancel()], 
...
while True:
    ...
    if event == 'Submit':
        ...
        try:
            ...
            while True:
                if chet > 0:
                    print("Выберети новую роль и протокол \n\n")
                    event,values = window.read()
                    if event == 'Submit':
                        role=values[0]
                        protokol=values[1]
                ...
                elif protokol =='Экспоненциальный протокол подбрасывнания монеты':
                        if role == 'Alice':
